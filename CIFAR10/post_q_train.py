...
parser.add_argument('--w_bits', default=5, type=int, help='the bit-width of the quantized network')
parser.add_argument('--a_bits', default=5, type=int, help='the bit-width of the quantized network')
...
def main():
    ...
    if use_gpu:
        ...
        for m in model.modules():
            if hasattr(m, '_quantization'):
                m.q_method = "biscaled" # default q_method set as biscaled
                m._quantization()
    ...
    if not os.path.exists('result'):
        os.makedirs('result')
    fdir = 'custom_result/'+str(args.arch) + '_w' + str(args.w_bits) + '_a' + str(args.a_bits)
    ...
    init_prec = validate(testloader, model, criterion)
    print("INITIAL PRECISION:", init_prec)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        post_q_train(trainloader, model, criterion, optimizer, epoch)

        # evaluate on test set
        prec = validate(testloader, model, criterion)
        print("AFTER 1 EPOCH TRAINING PREC", prec)
...
def post_q_train(trainloader, model, criterion, optimizer, epoch):
